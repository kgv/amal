...
use crate::app::{
    computers::{DifferenceComputed, DifferenceKey},
    data::{save, Format},
    localize,
};
use anyhow::Result;
...
impl Pane {
    ...
    pub(super) fn header(&mut self, ui: &mut Ui) {
        ...
        ui.menu_button(RichText::new(FLOPPY_DISK).heading(), |ui| {
            ...
            if ui.button("CSV").clicked() {
                ...
                // if let Err(error) = self.save("df.csv", Format::Csv) {
                //     error!(%error);
                // }
            }
    ...
    fn window(&mut self, ui: &mut Ui) {
        Window::new(format!("{GEAR} Difference settings"))
            .id(ui.next_auto_id())
